...
fix_database_permissions () {
    ...
    mysql -h $DB_HOST -u root -p${DB_ROOT_PASSWORD} -e "flush privileges;"
    mysql -h $DB_HOST -u root -p${DB_ROOT_PASSWORD} -e "SET PASSWORD FOR 'koha_$LIBRARY_NAME'@'%' = PASSWORD('$MASTER_PASSWORD');"
    mysql -h $DB_HOST -u root -p${DB_ROOT_PASSWORD} -e "flush privileges;"
}
...
if [ ! -f /etc/configured ]; then
    ...
    then
        ...
        # issue:
        # failed to load external entity "/etc/koha/sites/$LIBRARY_NAME/koha-conf.xml"
        # date && time koha-mysql $LIBRARY_NAME < koha.sqlservice apache2 reload
        ...
        #koha-create --populate-db koha_$LIBRARY_NAME
    fi
    fix_database_permissions
    ...
    # Change password of master user
    LINE=$(grep -n '<pass>' /etc/koha/sites/$LIBRARY_NAME/koha-conf.xml | cut -d ':' -f1)
    sed -i "$LINE d" /etc/koha/sites/$LIBRARY_NAME/koha-conf.xml
    sed -i "$LINE i\ <pass>$MASTER_PASSWORD</pass>" /etc/koha/sites/$LIBRARY_NAME/koha-conf.xml

    echo "SET PASSWORD FOR 'koha_$LIBRARY_NAME' = PASSWORD('$MASTER_PASSWORD');" | mysql -u root --password=$DB_ROOT_PASSWORD -h $DB_HOST mysql
    echo "flush privileges;" | mysql -u root --password=$DB_ROOT_PASSWORD -h $DB_HOST
else
    # 2nd+ executions
